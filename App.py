...
st.markdown("### 📈 Equity Portfolio – Paolo (Range Sbloccato)")
...
if uploaded_files:
    ...
    if selected_names:
        ...
        # Definiamo limiti ampi per il calendario (dal 2015 al 2030)
        min_calendario = date(2015, 1, 1)
        max_calendario = date(2030, 12, 31)
        
        # Impostiamo il valore di default sulla data minima dei file, 
        # ma permettiamo all'utente di scegliere date diverse
        data_min_file = min(all_dates).date()
        data_max_file = max(all_dates).date()

        start_d = st.sidebar.date_input(
            "Inizio Analisi", 
            value=data_min_file,
            min_value=min_calendario,
            max_value=max_calendario
        )
        ...
        # Filtro basato sulla scelta utente
        mask = (df_port['date'].dt.date >= start_d) & (df_port['date'].dt.date <= end_d)
        df_periodo = df_port[mask].copy()

        if df_periodo.empty:
            st.warning("⚠️ Nessun dato presente nel range selezionato. Prova a cambiare le date.")
        else:
            # Ricalcolo Equity da 0 per il periodo
            for name in selected_names:
