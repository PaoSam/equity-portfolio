...
if uploaded_files:
    ...
    if raw_data:
        ...
        if selected_names:
            ...
            # Matrice per posizioni e testi tooltip
            net_exposure = {d: {} for d in dates_set}
            active_info = {d: [] for d in dates_set} # Per il tooltip delle strategie attive
            ...
            for name in selected_names:
                ...
                # Riempimento logica netting e info attive
                for d in dates_set:
            ...
            fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.04, 
                               ...
                               subplot_titles=("Equity (Totale e Singole)", "Drawdown Portafoglio ($)", "Margine Reale ($) - Passa il mouse per i dettagli"))
            ...
            # Margine con Tooltip delle strategie attive
            fig.add_trace(go.Scatter(
                x=df_master['date'], 
                y=df_master['Margine_Reale'], 
                name='Margine', 
                fill='tozeroy', 
                line=dict(color='orange'),
                text=df_master['Info_Attive'],
                hovertemplate="<b>Margine:</b> $%{y:,.0f}<br><b>Strategie attive:</b><br>%{text}<extra></extra>"
            ), row=3, col=1)
