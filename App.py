...
if uploaded_files:
    ...
    if raw_data:
        ...
        if selected_names:
            ...
            stats_list = []
            active_info = {d: [] for d in dates_set} # Per il tooltip margine
            ...
            for name in selected_names:
                ...
                # Calcolo della curva equity singola per il grafico
                df_master[f'eq_{name}'] = df_master[f'pnl_{name}'].cumsum()
                
                # Calcolo Statistiche Trade
            ...
            for d in dates_set:
                for name in selected_names:
                    ...
                    net_exposure[d][ticker_map[name]] += pos_val
                    if pos_val != 0:
            ...
            fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.04, 
                               ...
                               subplot_titles=("Equity (Totale in Nero, Singole in Colore)", "Drawdown ($)", "Margine Reale ($)"))
            ...
            # 1b. Equity Singole (Ripristinate)
            for name in selected_names:
                fig.add_trace(go.Scatter(x=df_master['date'], y=df_master[f'eq_{name}'], name=name, line=dict(width=1.2), opacity=0.5), row=1, col=1)
            ...
            fig.add_trace(go.Scatter(x=df_master['date'], y=df_master['Margine_Reale'], name='Margine', fill='tozeroy', line=dict(color='orange'),
                                     text=df_master['Info_Attive'], hovertemplate="Margine: $%{y:,.0f}<br>Attive:<br>%{text}<extra></extra>"), row=3, col=1)
            ...
            with c1:
                st.write("### 📊 Statistiche Strategie")
            ...
            with c2:
                ...
                days = (end_date - start_date).days
                total_mar = (total_pnl / days * 365) / total_dd if total_dd != 0 and days > 0 else 0
                st.write("### 🏆 Portfolio Index")
                st.metric("Portfolio MAR Ratio", f"{total_mar:.2f}")
                st.metric("Max DD Portafoglio", f"${total_dd:,.0f}")
